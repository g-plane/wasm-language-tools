...
use wat_syntax::{ast::GlobalType, SyntaxNode};
        ...
        symbol_table
            ...
            .or_else(|| {
                symbol_table
                    ...
                    .iter()
                    .find(|symbol| {
                        symbol.key == key && matches!(symbol.kind, SymbolItemKind::GlobalDef(..))
                    })
                    .map(|symbol| {
                        let root = self.ctx.root(uri);
                        Hover {
                            contents: HoverContents::Scalar(create_global_def_hover(symbol, &root)),
                            range: Some(helpers::rowan_range_to_lsp_range(
                                &line_index,
                                token.text_range(),
                            )),
