...
pub fn check(db: &dyn salsa::Database, diagnostics: &mut Vec<Diagnostic>, document: Document, line_index: &LineIndex) {
    diagnostics.extend(document.syntax_errors(db).iter().map(|error| Diagnostic {
        ...
        code: if let Message::Name(name) = error.message {
            Some(Union2::B(format!("{DIAGNOSTIC_CODE}/{}", name.replace(' ', "-"))))
        } else {
