...
pub fn check(diagnostics: &mut Vec<Diagnostic>, ctx: &mut DiagnosticCtx, node: AmberNode) {
    let severity = match ctx.config.lint.unreachable {
    ...
    let cfg = cfa::analyze(ctx.db, node.green().clone().into(), node.text_range());
