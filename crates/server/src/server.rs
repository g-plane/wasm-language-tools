...
impl Server {
            ...
            .or_else(|params| {
                try_cast_request::<RenameRequest>(&method, params).map(|params| {
                    params.and_then(|params| match service.rename(params) {
                        Ok(result) => serde_json::to_value(result).map(|result| Message::OkResponse { id: id.clone(), result }),
    ...
    fn handle_response(&mut self, id: RequestId, result: serde_json::Value) -> anyhow::Result<()> {
